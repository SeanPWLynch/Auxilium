...
public class currentAdminTicketTableModel extends AbstractTableModel
{
	...
	public currentAdminTicketTableModel(String currentAdmin)
	{
		this.currentAdmin = currentAdmin;
		try
		{
			// Connections.createConnection();
			Database.getCurrentAdminTickets(currentAdmin);
	...
	public static void filterRows(String ID, String departmentID, int priority)
	{
		...
		try
		{
			...
			if (priority >= 0)
			{
				queryString = "SELECT * FROM TICKET WHERE ADMINID = '"
						+ currentAdmin + "' AND departmentid like '"
						+ departmentID + "%' AND ticketID like '" + ID.trim()
			...
			} else
			{
				...
				queryString = "SELECT * FROM TICKET WHERE ADMINID = '"
						+ currentAdmin
						+ "' AND departmentid like '%' AND ticketID like '%' AND ticketpriority >= 0";
			}
