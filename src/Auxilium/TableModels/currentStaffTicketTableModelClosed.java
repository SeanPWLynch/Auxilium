...
/**
 * This class is a custom made table model to hold tickets created by a certain selected user that have been closed.
 * 
 * @author Gavin Kenna
 * @author Sean Lynch
...
public class currentStaffTicketTableModelClosed extends AbstractTableModel
{
	...
	public currentStaffTicketTableModelClosed(String currentStaff)
	{
		currentStaffTicketTableModelClosed.currentStaff = currentStaff;
		try
		{
			// Connections.createConnection();
			Database.getCurrentStaffClosedTickets(currentStaff);
	...
	public static void filterRows(String title)
	{
		try
		{
			...
			queryString = "SELECT TICKETID, TICKETTITLE, TICKETDESCRIPTION, DATECREATED, DATECLOSED, ADMINID FROM TICKET WHERE StaffId = '"
			        + currentStaff + "' AND tickettitle like '" + title + "%' AND DateClosed is not null";
